...
let { MaterialType, MachineType, SupplyType,
      WorkType, WorkforceType, TenderType, ClientType,
      Client, ListType, Machine, Supply, Material} = require('../models');
...
module.exports = {
  up: async(queryInterface, Sequelize) => {
      ...
      await queryInterface.bulkInsert('supply_types', dataTypes, { returning: true, validate: true})
      ...
      await values.forEach( async(data) => {
        ...
        delete data.type;
        await dataInsert.push(Object.assign({}, data, {
          type_id : type.id,
          created_at: new Date(),
          updated_at: new Date()
        }));
      });
      await queryInterface.bulkInsert('supplies', dataInsert, { returning: true, validate: true });
      // Tipo Materiales
      types = [{
      ...
      await queryInterface.bulkInsert('material_types', dataTypes, { returning: true, validate: true, });
      types = [{
          name: 'Perfiles Cerrados',
          id: 1,
        },{
      ...
      await queryInterface.bulkInsert('materials', dataInsert, { returning: true, validate: true});
      ...
      await queryInterface.bulkInsert('machine_types', dataTypes, { returning: true, validate: true, });
      ...
      await queryInterface.bulkInsert('machines', dataInsert, { returning: true, validate: true });
      ...
      usersIds.forEach( async(user) => {
        let aux = { user_id : user.id, role_id : 1 ,
          created_at : new Date(), updated_at: new Date() };
        dataProfiles.push(aux);
      });

      return queryInterface.bulkInsert('profiles',dataProfiles, { returning: true, validate: true });
  },
